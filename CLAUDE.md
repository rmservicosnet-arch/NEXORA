...
| Teste que procura a agulha numa PÁGINA | `limite=100` e um `find` pelo nome passa enquanto o banco é novo e falha no dia em que cabem 101. Busque pelo que identifica — `termo`, id —, nunca pela posição na lista |
| `R$ ${x.toFixed(2)}` numa frase em português | Sai "R$ 250.00" — ponto decimal, formato de outro país, na mensagem que alguém lê no balcão. `formatarBRL` existia em `@estoque/core` com teste, e 15 mensagens em 5 serviços o ignoravam |
| Dois caminhos para a mesma operação, e só um confere o retorno | O pagamento com saldo distinguia "excedeu o limite"; a venda A PRAZO, logo abaixo, descartava `excedeuLimite` e avisava a mesma coisa nos dois casos. O teste EXIGIA o aviso errado, então passava. Ao duplicar um caminho, duplique as conferências |
| Prévia de operação financeira calculada no navegador | Seria a regra de dinheiro escrita duas vezes, e a segunda envelhecendo em silêncio. A prévia da devolução roda o SERVIÇO inteiro e desfaz no fim: o que a tela mostra é literalmente o que vai acontecer, e as recusas aparecem antes de a pessoa apertar o botão |
| Indicador que soma só o status "feliz" | `totalVendido` somava as CONCLUIDA: uma venda com devolução parcial sumia inteira do faturamento, inclusive a parte que ficou com o cliente. Faturado é `total − devolvido`, sobre tudo o que não foi cancelado |
| Filtro cuja lista de opções não tem rota | A API aceita `vendedorId` e não há endpoint que liste vendedores. Derivar as opções da PÁGINA daria escolhas erradas a partir da segunda — o filtro sai da tela até a lista existir |
| `&&` depois de um comando que pode falhar sem importar | `rm -rf tmp && python patch.py` — o `rm` esbarrou num arquivo aberto e o patch simplesmente não rodou, mas o commit seguinte, em outra linha, entrou. Limpeza usa `;`, não `&&` |
| Devolver ao cliente que ainda não pagou | Devolução desce uma cascata na ordem INVERSA da venda: abate o título em aberto, depois o débito na carteira, e só o que sobrar volta como dinheiro. Devolver dinheiro a quem ainda deve é pagar duas vezes |
| Dinheiro que entrou por cartão ou PIX numa devolução | O estorno acontece na maquineta, fora do sistema. Sem carteira onde creditar, a API RECUSA e explica — gravar o número num lugar qualquer só para a operação passar é o dinheiro sem dono de novo |
| Número certo ao lado do número errado | `exportacoes` vinha de um `GROUP BY` sobre a janela e `linhasExportadas` de um `reduce` sobre a PÁGINA. Enquanto tudo cabia numa página os dois coincidiam; passado o `LIMIT`, exportar 42 linhas fazia o total subir 39. O indicador ao lado, correto, dava aval ao errado |
| Operação de dinheiro sem caminho de volta | Baixa de título não tinha estorno: digitou errado, ficou. E a recusa de cancelar uma venda mandava "estorne a baixa antes", porta que não existia. Toda operação que move dinheiro nasce com o contrário dela — e o contrário é LANÇAMENTO, não `delete` |
| Diálogo nativo do navegador numa tela que não usa nenhum | `window.prompt` não tem o estilo, nem o foco, nem o celular. Motivo se pede na própria tela |
| Teste fora do `tsc` | `apps/api`, `core`, `db` e `contracts` excluíam `*.test.ts` do build — e ficar fora do build virou ficar fora da CONFERÊNCIA. `npm run typecheck` dizia "limpo" com arquivo de teste que não compilava. Cada projeto tem `tsconfig.test.json` com `noEmit`, e `typecheck` roda os dois |
| Cópia local do contrato dentro do teste | É de propósito — comparar com o tipo do pacote esconderia campo que sumiu da resposta. Mas cópia que não acompanha mente ao contrário: faltavam `motivo` e `aceiteClienteEm`, e `expect(undefined).not.toBeNull()` PASSA. O teste do aceite não verificava nada |
| Cancelar desfazendo só metade do que a operação fez | Cancelar venda estornava o ESTOQUE e nada mais: a mercadoria voltava para a prateleira e a dívida ficava de pé. Quem cancela desfaz TODOS os efeitos — chamando cada módulo, nunca escrevendo na tabela dele |
| Chamar o recurso OPCIONAL como se fosse obrigatório | A baixa de título a receber chamava a carteira sempre, e `lancar` dá 404 para quem não tem: o título de venda a prazo nasce EXATAMENTE para o cliente sem carteira e não podia ser recebido em caminho nenhum. Cobrança impossível, sem erro de compilação |
| Mensagem de erro mandando apertar um botão que não existe | "Estorne a baixa antes de cancelar" — e estorno de baixa não é rota. Recusar sem explicar é ruim; mandar procurar uma porta que não há é pior |
| Restringir só o relatório que tem "custo" no nome | `compras/fornecedores` devolve valor e unidades lado a lado: dividir um pelo outro dá o custo unitário do fornecedor. `compras/a-receber` devolve o valor parado, que é o mesmo número antes de entrar no estoque. Os TRÊS de compras exigem `relatorio.ver_custo` — e o teste é `it.each` sobre os três, não sobre o óbvio |
| Indicador do desenho discordando da tabela do próprio desenho | O artboard dizia "Subiram 4" numa lista com 3 subindo. Mock também tem invariante: quem lê a proposta decide por ela, e um número que não fecha vira decisão errada antes de existir código |
| Tela construída sem artboard no canvas | O desenho deixa de ser a proposta e vira retrato antigo. Sete telas existiam no app e não no canvas, o menu do canvas não tinha a rota criada e o índice de relatórios listava como pendente o que já rodava. Tela nova pede artboard junto — e quem a INDEXA precisa apontar para ele |
